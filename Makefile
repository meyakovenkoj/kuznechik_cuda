CFLAGS = -std=c99 -I./include -O3 -Wall -Werror -pedantic -g -fPIC
...
NVLDFLAGS = -L /usr/local/cuda/lib64 -lcudart
EXECUTABLE := main.exe
CPU_TEST := cpu_test.exe
CPU_MAIN := cpu_main.exe
GEN_MAIN := gen.exe
CU_FILES   := cuda_helper.cu cuda_kuznechik.cu
CC_FILES   := main.c input.c kuznechik.c cpu_test.c ctr_encrypt.c
SRCDIR=src
OBJDIR=build
OBJS=$(OBJDIR)/input.o $(OBJDIR)/cuda_helper.o $(OBJDIR)/cuda_kuznechik.o $(OBJDIR)/main.o $(OBJDIR)/kuznechik.o $(OBJDIR)/ctr_encrypt.o
CPU_OBJS=$(OBJDIR)/kuznechik.o $(OBJDIR)/cpu_test.o $(OBJDIR)/cpu_kuznechik.o
LIB_OBJS=$(OBJDIR)/cpu_kuznechik.o $(OBJDIR)/kuznechik.o $(OBJDIR)/ctr_encrypt.o
CPU_TEST_OBJS=$(OBJDIR)/cpu_main.o $(OBJDIR)/input.o
...
$(OBJDIR)/ctr_encrypt.o: $(SRCDIR)/ctr_encrypt.c
	$(CC) $< $(CFLAGS) -DCPU_PROG -c -o $@
...
$(CPU_MAIN): $(CPU_TEST_OBJS)
	$(CC) -o $@ $(CPU_TEST_OBJS) libkuznechik.a

$(GEN_MAIN): $(SRCDIR)/generator.c $(SRCDIR)/input.c 
	$(CC) $(CFLAGS) $(GEN_SRC) -o $@

lib: $(LIB_OBJS)
	ar -rcs libkuznechik.a $(LIB_OBJS)
	ranlib libkuznechik.a
