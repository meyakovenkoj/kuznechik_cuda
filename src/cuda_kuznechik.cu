...
__global__ void ctr_encrypt(uint4 *data, const uint4 *ls_matrix, uint4 iv, unsigned int numblock)
{
    ...
    {
	ctr ^= *(sd_rkey + i);
        cur_result = *(ls_matrix + 16*( ctr.x & 0x000000ff)        + 0);
        cur_result ^= *(ls_matrix + 16*((ctr.x & 0x0000ff00) >> 8)  + 1);
        cur_result ^= *(ls_matrix + 16*((ctr.x & 0x00ff0000) >> 16) + 2);
        cur_result ^= *(ls_matrix + 16*((ctr.x & 0xff000000) >> 24) + 3);
        cur_result ^= *(ls_matrix + 16*( ctr.y & 0x000000ff)        + 4);
        cur_result ^= *(ls_matrix + 16*((ctr.y & 0x0000ff00) >> 8)  + 5);
        cur_result ^= *(ls_matrix + 16*((ctr.y & 0x00ff0000) >> 16) + 6);
        cur_result ^= *(ls_matrix + 16*((ctr.y & 0xff000000) >> 24) + 7);
        cur_result ^= *(ls_matrix + 16*( ctr.z & 0x000000ff)        + 8);
        cur_result ^= *(ls_matrix + 16*((ctr.z & 0x0000ff00) >> 8)  + 9);
        cur_result ^= *(ls_matrix + 16*((ctr.z & 0x00ff0000) >> 16) + 10);
        cur_result ^= *(ls_matrix + 16*((ctr.z & 0xff000000) >> 24) + 11);
        cur_result ^= *(ls_matrix + 16*( ctr.w & 0x000000ff)        + 12);
        cur_result ^= *(ls_matrix + 16*((ctr.w & 0x0000ff00) >> 8)  + 13);
        cur_result ^= *(ls_matrix + 16*((ctr.w & 0x00ff0000) >> 16) + 14);
        cur_result ^= *(ls_matrix + 16*((ctr.w & 0xff000000) >> 24) + 15);
        ctr = cur_result;
    }
    ctr ^= *(sd_rkey + 9);
    data[index] ^= ctr;
...
void encrypt_cuda(const uint8_t *inparray, uint8_t *outarray, const uint8_t *ls_matrix, const uint8_t *rkey, const uint8_t *iv, const uint32_t numblock)
{
    ...
    for (int device_id = 0; device_id < config.deviceNum; device_id++)
    {
        ...
        CUDA_ERROR();
        cudaMemcpy(ddata, block_array, sizeof(uint8_t) * num_uint8_ts, cudaMemcpyHostToDevice);
        CUDA_ERROR();
